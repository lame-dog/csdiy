...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        env.bindings[expressions.first] = scheme_eval(
        ...
        "*** YOUR CODE HERE ***"
        vars = signature.rest
        if isinstance(vars, Link):
            while vars != nil:
                if not isinstance(vars.first, str):
                    raise SchemeError()
                vars = vars.rest
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if isinstance(expressions.first, Link):
            sub = scheme_eval(expressions.first, env)
        else:
            sub = expressions.first
        if isinstance(sub, str) and is_scheme_false(env.lookup(sub)):
            return False
        if is_scheme_false(sub):
            return False
...
def do_or_form(expressions, env):
    ...
    while expressions != nil:
        if isinstance(expressions.first, Link):
            ...
            sub = expressions.first
        if isinstance(sub, str) and sub in env.bindings.keys() and is_scheme_true(env.lookup(sub)):
            return sub
        if is_scheme_true(sub):
            return sub
        expressions = expressions.rest
    return False
    # END PROBLEM 12
