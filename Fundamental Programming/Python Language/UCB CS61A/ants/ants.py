...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        i = 0
        while p:
            if not p.is_hive and i >= self.lower_bound and i <= self.upper_bound:
                if len(p.bees):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        super().reduce_health(min(amount, self.health))
        # END Problem 5
...
class Water(Place):
    ...
    def add_insect(self, insect: Insect):
        ...
        if self.ant and not self.ant.is_waterproof:
            self.ant.reduce_health(self.ant.health)
        for b in self.bees[:]:
            if not b.is_waterproof:
                b.reduce_health(b.health)
        # END Problem 10
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        for p in gamestate.places.values():
            if p == self.place:
                break
            if p.ant:
                p.ant.double()
