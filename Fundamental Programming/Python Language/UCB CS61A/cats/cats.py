...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    cand = [p for p in paragraphs if select(p)]
    if len(cand) < k + 1:
        return ""
    return cand[k]
    # END PROBLEM 1
...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return lambda s: any([k in remove_punctuation(lower(s)).split() for k in keywords])
    # END PROBLEM 2
...
def accuracy(typed: str, source: str) -> float:
    ...
    correct = sum([typed_words[i] == source_words[i] for i in range(min(ls, lt))])
    return 100.0 * correct / lt
...
def autocorrect(
    ...
    if typed_word in word_list:
        return typed_word
    new_diff = lambda w: diff_function(typed_word, w, limit)
    word = min(word_list, key=new_diff)
    if new_diff(word) > limit:
        return typed_word
    return word
    # END PROBLEM 5
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for j in word_indices:
        not_taked = True
        time = [get_time(times, _, j) for _ in player_indices]
        for i in player_indices:
            if not_taked and time[i] == min(time):
                not_taked = False
                fastest[i].append(words[j])
    return fastest
    # END PROBLEM 10
