...
def interleaved_sum(n, f_odd, f_even):
    ...
    def do_sum(k, f_odd, f_even):
        if k > n:
            return 0
        return f_odd(k) + do_sum(k + 1, f_even, f_odd)

    return do_sum(1, f_odd, f_even)
...
def count_dollars(total):
    ...
    def dollar_partition(n, bill):
        if bill == 1:
            return 1
        if n < bill:
            return dollar_partition(n, next_smaller_dollar(bill))

        return dollar_partition(n - bill, bill) + dollar_partition(
            n, next_smaller_dollar(bill)
        )

    return dollar_partition(total, 100)
...
def count_dollars_upward(total):
    ...
    def partition(n, bill):
        ...
        if n < 0:
            return 0
        if n < bill:
            return partition(n - bill, bill)
        if n > bill:
            return partition(n - bill, bill) + partition(n, next_larger_dollar(bill))
